...
export class ClaudeSessionScanner {
   ...
   */
  getRecentSessions(limit: number = 10): ExistingClaudeSession[] {
    ...
    try {
      ...
      const sessionMap = new Map<string, ExistingClaudeSession>();
   ...
   */
  private getSessionFilePath(project: string, sessionId: string): string {
  ...
  getProjectSessions(projectPath: string): ExistingClaudeSession[] {
    ...
    try {
      ...
      const sessions: ExistingClaudeSession[] = [];
